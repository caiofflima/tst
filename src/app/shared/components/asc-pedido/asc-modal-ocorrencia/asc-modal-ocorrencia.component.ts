...
import { DocumentoTipoProcesso, TipoOcorrencia } from 'app/shared/models/entidades';
...
import { FileUploadService, SessaoService, TipoOcorrenciaService } from 'app/shared/services/services';
...
export class AscModalOcorrenciaComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('modalComponent', { static: true }) private modalComponent!: TemplateRef<any>;
  ...
  get tipoOcorrenciaValue() {
    return this.form.get("tipoOcorrencia");
  }

  get idTipoOcorrenciaValue(): AbstractControl {
    return this.form.get("idTipoOcorrencia");
  }
    ...
    const dialogRef2 = this._dialogService.confirm({
      data: {
        ...
        actionButton: {
          ...
          confirmFunction: () => this.adicionarOcorrencia()
        }
      }
    });
  }
  ...
  getServiceObservable(p?: any) {
    this.tipoOcorrenciaService
    .consultarTiposOcorrenciaManuais()
    .subscribe((tipos: TipoOcorrencia[]) => {
      this.options = tipos.map(to => ({
        label: to.nome,
        value: to
      }));
    });
  ...
  change(event: any): void {
    ...
    setTimeout(() => {
      ...
      this.tipoOcorrenciaValue.setValue(event.value);
    }, 20);
  }
}
