...
@Component({
    ...
    styleUrls: ['./pesquisar-processo-reembolso-form.component.scss']
})
export class PesquisarProcessoReembolsoFormComponent extends BaseComponent {
    ...
    public salvar(): void {
        ...
        if (!this.patologia.id) {
            ...
            this.patologiaService.alterar(patologia).subscribe(async res => {
                ...
                await this.router.navigate(['/manutencao/pesquisar-processo-reembolso/buscar'], {queryParams: {codigo: res.codigo}});
            }, err => this.showDangerMsg(err.error));
        }
    }
    ...
    public excluir() {
        this.messageService.addConfirmYesNo(this.bundle("MA021"), () => {
            this.patologiaService.excluir(this.id).subscribe(async () => {
                    ...
                    await this.router.navigate(['/manutencao/pesquisar-processo-reembolso/buscar']);
                }, err => this.showDangerMsg(err.error)
            );
        });
    }
